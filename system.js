...
let count = 1;
let members = []; //등록된 멤버 저장소(배열)
let findMember = []; //검색된 멤버 저장소(배열)
let id = 0; //id는 0
let changeMember;
...
function delete_f() {
    ...
    for (let i = 0; i < delRow.length; i++) { //반복문 시작
        delRow[i].addEventListener('click', (event) => { //delRow라는 버튼을 누르면
            const delTarget = event.target;
            delTarget.parentNode.parentNode.remove(); //tr->td로 들어가 삭제
            let delNumber = delTarget.dataset.id; //data-id값 뽑아오기
...
function revise_f() {
    ...
    for (let i = 0; i < changeBtn.length; i++) {
        changeBtn[i].addEventListener('click', (event) => {
            ...
            let changeNum = changeTarget.dataset.id; //date-id 값
...
//검색한 객체에서 수정하기
function searchMem() {
    const changeBtn = document.getElementsByClassName('revise'); //changeBtn 변수 선언
    for (let i = 0; i < changeBtn.length; i++) {
        changeBtn[i].addEventListener('click', (event) => {
            const changeTarget = event.target;
            let changeNum = changeTarget.dataset.id; //date-id 값
...
register.addEventListener('click', () => {
    ...
    localStorage.setItem('members', JSON.stringify(members)); //등록된 값 저장
});
...
// 초기화 이벤트
initBtn.addEventListener('click', memberInit);
